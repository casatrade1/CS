...
3. **중요**: Settings → General → Root Directory를 `cs-reply-assistant`로 설정 (레포 루트가 아니라 이 폴더가 루트)
4. Framework는 자동으로 Next.js 인식
5. (선택) Gemini를 쓰려면 Vercel 프로젝트 Settings → Environment Variables에 아래 추가
   - `GEMINI_API_KEY`: Google AI Studio에서 발급한 API Key
   - `GEMINI_MODEL`: (선택) 기본값 `gemini-1.5-flash`
6. Deploy 클릭

> **404 에러가 나면**: Root Directory가 `cs-reply-assistant`로 설정되어 있는지 확인하세요.
